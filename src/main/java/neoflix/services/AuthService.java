        ...
        try (var session = driver.session()) {
            var user = session.executeWrite(
                    tx -> {
                        ...
                        var res = tx.run(
                                statement,
                                Values.parameters("email", email, "encrypted", encrypted, "name", name));
...
//        var foundUser = users.stream().filter(u -> u.get("email").equals(email)).findAny();
        try (var session = driver.session()) {
            ...
            if (
                    !plainPassword.equals(user.get("password"))
                            &&
